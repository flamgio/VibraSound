...
const demoAnalyses = [
  {
    ...
    bpm: 112.4, key: "Bb Major", energy: 0.74, danceability: 0.62,
    dominant_frequency: 528,
    frequency_spectrum: JSON.stringify([
    ...
    beat_pattern: JSON.stringify([0.9,0.3,0.7,0.3,0.9,0.3,0.7,0.3,0.9,0.3,0.7,0.3,0.9,0.3,0.7,0.3]),
    tempo_changes: JSON.stringify([{timestamp:0,bpm:111.8},{timestamp:45,bpm:112.4},{timestamp:130,bpm:112.6}]),
    cellular_resonance: JSON.stringify({
      score: 72, category: "beneficial",
    ...
    mood_genre: JSON.stringify({
      mood: "Melancholic", moodEmoji: "🌧", moodConfidence: 0.87,
      moodDescription: "Deep emotional weight with cathartic release. Tension-driven verses give way to soaring choruses.",
      genre: "Alternative Rock", subGenre: "Nu-Metal", genreConfidence: 0.92,
  ...
  },
  {
    ...
    bpm: 60.0, key: "A Minor", energy: 0.21, danceability: 0.18,
    ...
    beat_pattern: JSON.stringify([0.4,0.1,0.2,0.1,0.4,0.1,0.2,0.1,0.4,0.1,0.2,0.1,0.4,0.1,0.2,0.1]),
    tempo_changes: JSON.stringify([{timestamp:0,bpm:65},{timestamp:120,bpm:62},{timestamp:300,bpm:60}]),
    cellular_resonance: JSON.stringify({
      score: 97, category: "highly_beneficial",
    ...
    mood_genre: JSON.stringify({
      mood: "Calm", moodEmoji: "🌊", moodConfidence: 0.98,
      ...
      genre: "Ambient", subGenre: "Therapeutic Drone", genreConfidence: 0.95,
  ...
  },
  {
    ...
    bpm: 96.0, key: "C# Minor", energy: 0.66, danceability: 0.83,
    ...
    beat_pattern: JSON.stringify([0.9,0.4,0.6,0.4,0.8,0.4,0.6,0.4,0.9,0.4,0.6,0.4,0.8,0.4,0.6,0.4]),
    tempo_changes: JSON.stringify([{timestamp:0,bpm:96},{timestamp:60,bpm:96},{timestamp:200,bpm:96}]),
    cellular_resonance: JSON.stringify({
      score: 81, category: "highly_beneficial",
    ...
    mood_genre: JSON.stringify({
      mood: "Groovy", moodEmoji: "💃", moodConfidence: 0.91,
      moodDescription: "Infectious, dance-floor ready groove. The marimba-driven melody creates an irresistible rhythmic pattern.",
      genre: "Pop", subGenre: "Tropical Pop", genreConfidence: 0.88,
  ...
  },
  {
    ...
    bpm: 116.0, key: "F# Minor", energy: 0.78, danceability: 0.91,
    ...
    beat_pattern: JSON.stringify([1.0,0.3,0.6,0.3,1.0,0.3,0.6,0.3,1.0,0.3,0.6,0.3,1.0,0.3,0.6,0.3]),
    tempo_changes: JSON.stringify([{timestamp:0,bpm:116},{timestamp:90,bpm:116},{timestamp:210,bpm:116}]),
    cellular_resonance: JSON.stringify({
      score: 88, category: "highly_beneficial",
    ...
    mood_genre: JSON.stringify({
      mood: "Euphoric", moodEmoji: "✨", moodConfidence: 0.94,
      moodDescription: "Pure euphoric disco-funk. The hypnotic chord progression creates a trance-like state of joyful anticipation.",
      genre: "Electronic", subGenre: "Nu-Disco / Funk", genreConfidence: 0.93,
  ...
  },
  {
    ...
    bpm: 122.4, key: "C Minor", energy: 0.69, danceability: 0.72,
    dominant_frequency: 528,
    frequency_spectrum: JSON.stringify([
    ...
    beat_pattern: JSON.stringify([0.9,0.3,0.5,0.7,0.9,0.3,0.5,0.7,0.9,0.3,0.5,0.7,0.9,0.3,0.5,0.7]),
    tempo_changes: JSON.stringify([{timestamp:0,bpm:121},{timestamp:55,bpm:122.4},{timestamp:200,bpm:122.8}]),
    cellular_resonance: JSON.stringify({
      score: 76, category: "beneficial",
      assessment: "Rich harmonic content with strong 528 Hz presence. Gospel-influenced vocal harmonics create powerful resonance patterns that uplift cellular energy.",
      healingFrequencies: [
    ...
    mood_genre: JSON.stringify({
      mood: "Energetic", moodEmoji: "⚡", moodConfidence: 0.85,
      moodDescription: "Driving, anthemic energy with gospel roots. Building from quiet verses to explosive choruses.",
      genre: "Pop", subGenre: "Indie Pop", genreConfidence: 0.82,
  ...
  },
  {
    ...
    bpm: 56.0, key: "D Major", energy: 0.44, danceability: 0.22,
    ...
    beat_pattern: JSON.stringify([0.7,0.1,0.3,0.1,0.7,0.1,0.3,0.1,0.7,0.1,0.3,0.1,0.7,0.1,0.3,0.1]),
    tempo_changes: JSON.stringify([{timestamp:0,bpm:40},{timestamp:60,bpm:48},{timestamp:180,bpm:56}]),
    cellular_resonance: JSON.stringify({
      score: 91, category: "highly_beneficial",
    ...
    mood_genre: JSON.stringify({
      mood: "Dreamy", moodEmoji: "🌌", moodConfidence: 0.96,
      moodDescription: "Transcendent, consciousness-expanding soundscape. The Shepard tone creates an ever-ascending illusion that dissolves boundaries of time.",
      genre: "Orchestral", subGenre: "Cinematic / Neoclassical", genreConfidence: 0.97,
...
async function seed() {
  ...
  try {
    const { rows } = await client.query("SELECT COUNT(*) as count FROM analyses");
    ...
    for (const demo of demoAnalyses) {
      await client.query(
        `INSERT INTO analyses (url, title, bpm, key, energy, danceability, dominant_frequency, frequency_spectrum, beat_pattern, tempo_changes, cellular_resonance, mood_genre, created_at)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9::jsonb,$10::jsonb,$11::jsonb,$12::jsonb,${demo.created_at})`,
        [demo.url, demo.title, demo.bpm, demo.key, demo.energy, demo.danceability,
         demo.dominant_frequency, demo.frequency_spectrum, demo.beat_pattern,
         demo.tempo_changes, demo.cellular_resonance, demo.mood_genre]
      );
